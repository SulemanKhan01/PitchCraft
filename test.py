...
print("--- Step 1: Generating Cover Letter Content ---")
try:
    gen_url = f"{BASE_URL}/cover-letter"
    response = requests.post(gen_url, json={"jd_text": jd_text})
    print(f"Content Gen Status Code: {response.status_code}")
    ...
    if response.status_code == 200:
        ...
        print("--- Step 2: Generating PDF from Content ---")
        pdf_url = f"{BASE_URL}/cover-letter/pdf"
        
        # We send the generated content text to the PDF endpoint
        pdf_response = requests.post(pdf_url, json={"text": generated_content})
        print(f"PDF Gen Status Code: {pdf_response.status_code}")
        
        if pdf_response.status_code == 200:
            filename = "cover_letter.pdf"
            # Write the streaming bytes to a local PDF file
            with open(filename, "wb") as f:
                f.write(pdf_response.content)
            print(f"Success! PDF saved locally as '{filename}'")
        else:
